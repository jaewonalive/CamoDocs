...
# Run it directly:
#   bash scripts/eval_query_detection.sh
#
...
#          -J querydetect -o logs/out.%j scripts/eval_query_detection.sh
#
# Requires 1 GPU.

# Main-table defense — Query Detection (per-document SequenceMatcher filter)
# vs. CamoDocs. HotpotQA, Llama-3.1-8B victim, contriever retriever.
# Fills the Query Detection row of the main defense table.
...
EXP_ID_NUM=${EXP_ID_NUM:-eval_query_detection-}
...
DEFEND_METHOD="none"
REMOVAL_METHOD="query_detection"
...
LOG_NAME=$EXP_ID_NUM-$EVAL_DATASET-$EVAL_MODEL_CODE-$MODEL_NAME_FOR_LOG-camodocs-querydetect-M$M-rep$REPEAT_TIMES-$NOTE.log
...
# Query Detection knobs (per appendix Algorithm 1):
# disable exact-substring shortcut; flag any document whose best
# SequenceMatcher score against the query exceeds 0.8.
EXACT_MATCH=False
