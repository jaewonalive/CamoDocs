...
# Run it directly:
#   bash scripts/eval_isolation_forest.sh
#
...
#          -J isoforest -o logs/out.%j scripts/eval_isolation_forest.sh
#
# Requires 1 GPU.

# Main-table defense — Isolation Forest (sklearn anomaly detector on
# retriever embeddings, contamination=0.4) vs. CamoDocs.
# HotpotQA, Llama-3.1-8B victim, contriever retriever.
...
EXP_ID_NUM=${EXP_ID_NUM:-eval_isolation_forest-}
...
DEFEND_METHOD="none"
REMOVAL_METHOD="isolation_forest"
...
LOG_NAME=$EXP_ID_NUM-$EVAL_DATASET-$EVAL_MODEL_CODE-$MODEL_NAME_FOR_LOG-camodocs-isoforest-M$M-rep$REPEAT_TIMES-$NOTE.log
...
python3 -u evaluate.py \
        ...
        --isolation_forest_contamination $CONTAM_RATIO \
